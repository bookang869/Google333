// Kyle Boo kb0531@cs.washington.edu Copyright Kyle Boo 2024s
...
static void GetPortAndPath(int argc,
  ...
  uint16_t port_num = static_cast<uint16_t>(std::stoi(argv[1]));
  if (port_num < 0 || port_num > 65535) {
    cerr << "Invalid port number provided." << endl;
    Usage(argv[0]);
  }
  *port = port_num;
  ...
  if (stat(argv[2], &path_stat) != 0) {
    cerr << "Provided path is not a readable directory" << endl;
    Usage(argv[0]);
  }
  *path = argv[2];

  for (int i = 3; i < argc; i++) {
    string indexFile =argv[i];
    if (stat(indexFile.c_str(), &path_stat) != 0) {
      cerr << "Index file " << indexFile << " is not a readable file." << endl;
      Usage(argv[0]);
    }
    indices->push_back(indexFile);
  }
}
