...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  if (realpath(root_dir.c_str(), root_dir_path) == NULL) {
    std::cerr << "Failed to get the absolute path." << std::endl;
    return false;
  }

  char test_file_path[PATH_MAX];
  if (realpath(test_file.c_str(), test_file_path) == NULL) {
    std::cerr << "Failed to get the absolute path." << std::endl;
    return false;
  }

  std::string root_dir_str = std::string(root_dir_path) + "/";
  std::string test_file_str = std::string(test_file_path);

  if (test_file_str.find(root_dir_str) == std::string::npos) {
    return false;
  } else {
    return true;
  }
